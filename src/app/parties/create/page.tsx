...
export default function CreatePartyPage() {
  ...
  return (
    <div className="animate-in-fade" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: '64px 16px 130px', fontFamily: 'var(--font-sans)' }}>
          ...
          <form ref={formRef} style={{ display: 'flex', flexDirection: 'column', minHeight: 280 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: step === 1 ? 'block' : 'none' }}>
                <h2 style={{ margin: '0 0 20px', fontSize: 18, fontWeight: 600, color: 'var(--foreground)', textAlign: 'center' }}>Où voulez-vous jouer ?</h2>
              ...
              <div style={{ display: step === 2 ? 'block' : 'none' }}>
                <h2 style={{ margin: '0 0 6px', fontSize: 18, fontWeight: 600, color: 'var(--foreground)', textAlign: 'center' }}>Inviter des joueurs</h2>
                <p style={{ margin: '0 0 20px', fontSize: 12, color: 'var(--muted-foreground)', textAlign: 'center' }}>Optionnel · Ajoutez jusqu&apos;à 2 joueurs de votre club</p>
                      ...
                      <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 14, backgroundColor: 'var(--muted)', border: '1.5px solid rgba(232,112,58,0.4)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          {p.photo_url ? (
                            ...
                            <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: 'var(--muted-foreground)' }}>
                              {p.prenom?.charAt(0)}
                            </div>
                          )}
                          <div>
                ...
                {invitedPlayers.length < 2 && (
                  <div style={{ position: 'relative' }}>
                    <div style={{ position: 'relative' }}>
                      <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke='var(--muted-foreground)' strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)' }}>
                        ...
                        {searchResults.map(player => (
                          ...
                          >
                            {player.photo_url ? (
                              ...
                              <div style={{ width: 32, height: 32, borderRadius: '50%', backgroundColor: 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: 'var(--muted-foreground)' }}>
                    ...
                    {playerQuery.length >= 2 && searchResults.length === 0 && !isSearching && (
                      <p style={{ margin: '12px 0 0', textAlign: 'center', fontSize: 13, color: 'var(--muted-foreground)' }}>Aucun joueur trouvé dans ce club</p>
                    )}
                  </div>
                )}

                {invitedPlayers.length >= 2 && (
                  <p style={{ margin: 0, textAlign: 'center', fontSize: 12, color: 'var(--muted-foreground)', padding: '8px 0' }}>Maximum 2 joueurs invités atteint</p>
                )}
              </div>

              {/* Step 3: Date + Heure */}
              <div style={{ display: step === 3 ? 'block' : 'none' }}>
                <h2 style={{ margin: '0 0 20px', fontSize: 18, fontWeight: 600, color: 'var(--foreground)', textAlign: 'center' }}>Quand ?</h2>
              ...
              <div style={{ display: step === 5 ? 'flex' : 'none', flexDirection: 'column', gap: 20 }}>
                ...
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: 'var(--foreground)', textAlign: 'center' }}>Derniers détails</h2>
                <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 28, paddingTop: 20, borderTop: '1px solid #2C2C2E' }}>
              {step > 1 ? (
                <button type="button" onClick={prevStep} style={{ height: 48, padding: '0 24px', borderRadius: 100, border: '1px solid #3A3A3C', background: 'transparent', color: 'var(--foreground)', fontSize: 14, fontWeight: 500, fontFamily: 'var(--font-sans)', cursor: 'pointer' }}>Retour</button>
              ) : (
                <Link href="/" style={{ textDecoration: 'none' }}>
                  <button type="button" style={{ height: 48, padding: '0 24px', borderRadius: 100, border: '1px solid #3A3A3C', background: 'transparent', color: 'var(--muted-foreground)', fontSize: 14, fontWeight: 500, fontFamily: 'var(--font-sans)', cursor: 'pointer' }}>Annuler</button>
                </Link>
              )}
              {step < TOTAL_STEPS ? (
