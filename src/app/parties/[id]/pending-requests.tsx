...
export function PendingRequests({ partyId, pendingPlayers }: { partyId: string, pendingPlayers: PendingPlayer[] }) {
  ...
  return (
    <div style={{ background: '#1C1C1E', borderRadius: 28, padding: '24px 22px', marginBottom: 16, border: '1.5px solid rgba(232,112,58,0.3)' }}>
      <h2 style={{ margin: '0 0 16px', fontSize: 17, fontWeight: 600, color: '#E8703A', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {pendingPlayers.map(player => (
          <div key={player.user_id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#2C2C2E', padding: '12px 14px', borderRadius: 16 }}>
            {player.photo_url ? (
            ...
            <div style={{ flex: 1 }}>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 600, color: '#fff' }}>{player.prenom} {player.nom?.charAt(0)}.</p>
              <p style={{ margin: '2px 0 0', fontSize: 12, color: '#E8703A', fontWeight: 600 }}>Niveau: {player.niveau || 'N/A'}</p>
            </div>
