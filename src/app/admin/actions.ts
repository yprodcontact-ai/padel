...
export async function uploadBroadcastLogo(formData: FormData) {
  const supabase = createClient()
  ...
  const filePath = `broadcast-logo-${Date.now()}.${fileExt}`
  ...
  if (uploadError) {
    console.error('Error uploading broadcast logo:', uploadError)
...
export async function sendBroadcastMessage(
  ...
  for (const recipient of targetUsers) {
    ...
    const match = existingConvs?.find(c => {
      const conv = c.conversations as unknown as { title: string, is_read_only: boolean, logo_url: string | null }
