-- Permet la transition complete/confirmee -> publiee (réouverture quand un joueur quitte)
BEGIN;
...
BEGIN
    ...
    IF v_status NOT IN ('complete', 'confirmee') THEN
        RAISE EXCEPTION 'La partie doit être complète ou confirmée pour modifier le statut';
    END IF;

    IF p_status NOT IN ('confirmee', 'annulee', 'publiee') THEN
